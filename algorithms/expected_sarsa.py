...
def expected_sarsa(env, alpha=1, gamma=1, epsilon=0.1, N_episodes=1000,
        epsilon_decay=decay_none, alpha_decay=decay_none):
    """Determines action-value function for optimal policy with Expected SARSA

    Based on Sutton/Barto, Reinforcement Learning, 2nd ed. p. 133
    ...
    for i_episode in range(N_episodes):
        print("\r> Expected SARSA: Episode {}/{}".format(
        ...
        while not done:
            ...
            # Create expected value for state_new
            V_exp = 0
            pi_state_new = create_epsilon_soft_policy_state(
                Q, state_new, epsilon)
            for a in range(env.action_space.n):
                V_exp += pi_state_new[a]*Q[state_new][a]

            Q[state][action] += alpha_i*(reward
                + gamma*V_exp - Q[state][action])
            state = state_new
    print()
    return Q,history
